...
public class Main_Prager_3D {
	...
	public static void main(String[] args) {
		...
		File start = new File(Main_Prager_3D.class.getProtectionDomain().getCodeSource().getLocation().toURI().getPath());
        ...
        for(int l = 0;l<nlc;l++) { //Need to rewrite this for Prager only. Should export some of the initializing loops to be functions since this file is going to be too specialized for general purpose
			...
			//for(int d=0;d<3;d++) {
				for(int n = 0;n<iteration;n++) {
					double drift = 0.1/Math.sqrt(n+1);
							...
							for(double p = Math.random();p<t_theta;p++){
								...
								for(double q = Math.random();q<t_phi;q++){
									...
									if(nsd ==2) {
										...
										tmpxn[0][o] = tmpxn[0][o]+rx*delta*lMin[o];
										tmpxn[1][o] = tmpxn[1][o]+ry*delta*lMin[o];
									}else {
										rx = Math.cos(theta);
										...
										tmpxn[0][o] = tmpxn[0][o]+rx*delta*lMin[o];
										tmpxn[1][o] = tmpxn[1][o]+ry*delta*lMin[o];
										tmpxn[2][o] = tmpxn[2][o]+rz*delta*lMin[o];
									}
					...
					for(int o=0;o<nnp;o++) {
						if(moveable[o] == 0) {
							for(int e = 0;e<nsd;e++) {
								xn[e][o] = xn[e][o]+r_vec[o][e]*lMin[o]*drift;
			...
			if(check<10) {
				if(nsd == ndf) { //Truss Case
					...
					for(int e = 0;e<nel;e++) {
						...
						for(int n = 0;n<nen;n++) {
							for(int x = 0;x<nsd;x++) {
								workbook.getSheet("Topology").getRow(e).getCell(n*nsd+x+l*nen*nsd).setCellValue(Math.floor(xn[x][ien[e][n]]*10000)/10000);
								workbook.getSheet("Deformation").getRow(e).getCell(n*nsd+x+l*nen*nsd).setCellValue(Math.floor(coord[e][n][x]*10000)/10000);
							}
						}
